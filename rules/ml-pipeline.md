...
│
├── src/                        # 再利用を意識して切り出した py ファイル群、実行は ipynb からの呼び出しに限定
...
2. EDA
   1. エージェントが `project/notebooks/01_eda.ipynb` を生成し EDA (探索的データ解析) を実行
   2. エージェントが結果の詳細を `project/reports/eda.md` に保存
   3. ユーザーが 2.1~2.2 を納得するまで繰り返す
3. 前処理_1 (データクレンジング、欠損値補完)
   1. エージェントが `project/notebooks/02_data_cleaning.ipynb` を生成し前処理を実行
...
6. 前処理_2 (特徴量エンジニアリング)
   1. エージェントが `project/notebooks/03_main.ipynb` を生成し前処理を実行
...
7. モデリング
   1. エージェントが `project/notebooks/03_main.ipynb` を編集しモデリングを実行
   2. エージェントが学習済みモデルを `project/data/model.pkl` に保存
   3. エージェントが結果のログを `project/reports/experiment/[番号]-[アプローチの概要].md` に保存
8. 検証
   1. エージェントが `project/notebooks/03_main.ipynb` を生成し検証を実行
